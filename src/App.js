...
function App() {
  ...
  useEffect(()=>{
    const getGreeting = async()=>{ //to be able to use async and await in useEffect we need to create an async function.
      const response = await fetch("http://localhost:5005");//we fetch from our api server running on port 5001
  ...
  return (
    ...
    <h4 className='IdTitle'>Id</h4>
    <h4 className='EnglishTitle'>English</h4>
    <h4 className='IcelandicTitle'>Icelandic</h4>
    </div>
    <div>
      {greeting.map((item) => (
        <div key={item.id} className="row">
          <div className='id'>{item.id}</div>
        <div className='greeting'>{item.greeting}</div>
        <div className='greetingIcelandic'>{item.greetingIcelandic}</div>
</div>
      ))}
    </div>
    </div>
